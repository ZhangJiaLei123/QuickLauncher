...
public class UsbController {
	 ...
	 * @param context
	 */
	public UsbController(@NonNull Context context, IUsbConnState connectionHandler, int vid, int pid){
	...
	private void enumerate(IPermissionListener listener) {
		...
		while (deviter.hasNext()) {
			...
			l("发现设备: " + String.format("%04X:%04X", d.getVendorId(), d.getProductId()));

          //  Toast.makeText(mApplicationContext, "发现设备: " + String.format("%04X:%04X", starAnimationSync.getVendorId(), starAnimationSync.getProductId()), Toast.LENGTH_SHORT).show();
			DebugManage.WriteLog2("发现设备: " + String.format("%04X:%04X", d.getVendorId(), d.getProductId()));

            if (d.getVendorId() == VID && d.getProductId() == PID) {
            	bFound = true;
				l("设备号: " + d.getDeviceName());
				if (!mUsbManager.hasPermission(d))
                {
                    Toast.makeText(mApplicationContext, "enumerate, 权限返回失败" , Toast.LENGTH_SHORT).show();
                    listener.onPermissionDenied(d);
                }
				else{
                    QLog.i("enumerate, 开始连接识别器");
					//startHandler(starAnimationSync);
                    if(!GetConnInerface(d)){
                        QLog.e("UsbController","连接失败");
                    }
                    //TestComm(starAnimationSync);
					return;
				}
				break;
			}
		...
		if (bFound == false)
		{
			//Toast.makeText(mApplicationContext, "没有发现更多的设备" , Toast.LENGTH_SHORT).show();
			DebugManage.WriteLog2("没有发现更多的设备");
				...
				if (!intent.getBooleanExtra(
					...
					if (dev != null) {
						if (dev.getVendorId() == VID
                            ...
                            if(!GetConnInerface(dev)){
                                QLog.e("UsbController","连接失败");
                            }
                            //TestComm(dev);
						}
					} else {
						DebugManage.WriteLog2("设备不存在!");
                        mConnectionHandler.onDeviceNotFound();
					}
				}
			}
    ...
    private boolean GetConnInerface(UsbDevice dev){
        int n;
	...
	public final static String TAG = "USB控制器";
